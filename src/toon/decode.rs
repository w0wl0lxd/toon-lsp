...
mod scanner_driven {
    ...
    impl Parser<'_> {
        ...
        fn parse_object_entry(&mut self, map: &mut Map<String, Value>) -> DecodeResult<()> {
            ...
            if matches!(self.kind(), TokenKind::Newline | TokenKind::Eof | TokenKind::Dedent) {
                ...
                if matches!(self.kind(), TokenKind::Dedent) {
                    // The block is already closed (e.g. the last field in an
                    // expanded block). Emit an empty array for `key[0]:` forms.
        ...
        fn parse_expanded_item(&mut self) -> DecodeResult<Value> {
            ...
            // A dash with nothing on its line: empty object, unless an indented
            // child block follows. We peek for the child by checking for
            // Newline (peek past it): if Dedent or Dash follows the Newline, it
            // is an empty item; if Indent follows, it is a child block.
