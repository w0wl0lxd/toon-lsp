...
- Bumped crate version to `0.7.21` to avoid consuming additional `0.7.x`
  patch release numbers. No functional changes from `0.7.2`.
...
- **Key Folding (opt-in)**: `ToonConfig` gains `fold_keys` (encode) and
  `expand_paths` (decode) flags. When `fold_keys` is set, single-key object
  chains collapse into dotted paths on encode (e.g.
  `{"user":{"name":"x"}}` → `user.name: x`), and `expand_paths` restores them
  on decode so folded documents round-trip losslessly. Folding is off by
